...
class Bear(Critter):
    """Representation of a Bear critter in our world."""
    ...
    """Returns the color of a grizzly bear as brown and any other bears as white."""
    def get_color(self):
    ...
    """Returns True, the bear always wants to eat."""
    def eat(self):
        return True
    
    """Reutnr the attack to fight against an opponent as always Attack.SCRATCH."""
    def fight(self, opponent):
        return Attack.SCRATCH

    """Returns the bear’s movement to alternate between north and west."""
    def get_move(self, neighbors):
        if self.turn % 2 == 0:
    ...
    """Returns a string representation of this critter, “B”. This representation is used when fighting another critter."""
...
class Cheetah(Critter):
    """ Representation of a Cheetah critter in our world. """

    def __init__(self, location, hunger):
        super().__init__(location)
        self.hunger = hunger
        self.ate = True
        self.direction = random.choice([Direction.NORTH, Direction.SOUTH, Direction.WEST, Direction.EAST])
        self.counter = 0

    """Sets the color of a Cheetah to red."""
    def get_color(self):
        return "red"
    
    """Returns True at first then alternates between False and True until the cheetah eats the max amount of food it's allowed to eat (hunger)"""
    def eat(self):
        if self.hunger > 0:
            self.ate = True
        else:
    ...
    """Sets the attack fight against an opponent as scratch if it still has more food to eat and pounce if it already ate it's maximum amount of food."""
    def fight(self, opponent):
        if self.hunger > 0:
            return Attack.SCRATCH
        else:
    ...
    """Sets the cheetah's movement to randomly pick a direction and move in that direction for three moves until picking a new random direction."""
    def get_move(self, neighbors):
        if self.counter % 3 == 0:
            self.direction = random.choice([Direction.NORTH, Direction.SOUTH, Direction.WEST, Direction.EAST])
        self.counter += 1
    ...
    """ Returns a string representation that’s the number of pieces of food this Cheetah still wants to eat critter."""
    def __str__(self):
        return str(self.hunger)


class Lion(Critter):
    """Representation of a Lioncritter in our world."""
    def __init__(self, location):
        super().__init__(location)
        self.fought = False
        self.direction = []
        self.turn = 0
        direction_list = [Direction.SOUTH, Direction.WEST, Direction.NORTH, Direction.EAST]
        for d in direction_list:
            for i in range(5):
                self.direction.append(d)
            
    """Sets the color of a Lion to goldenrod3 (yellow)."""
    def get_color(self):
        return "goldenrod3"
    
    """Returns True, the lion wants to eat, if it just fought. If it didn’t just fight, returns False."""
    def eat(self):
        if self.fought == True:
            self.fought = False
            return True
    ...
    """Uses roar attack against bears and pounce attack for any other opponents. Keeps track of if the lion fought ot not for the eating method."""
    def fight(self, opponent):
        self.fought = True
        if opponent == "B":
            return Attack.ROAR
    ...
    """Sets the lion's movement to first go south 5 times, then go west 5 times, then go north 5 times, then go east 5 times (a clockwise square pattern), then repeat."""
    def get_move(self, neighbors):
        if self.turn <= 19:
    ...
    """ Returns a string representation of this critter, “L”. This representation is used when fighting another critter."""
    def __str__(self):
        return "L"


class Torero(Critter):
    """ Representation of a Torero critter in our world. """


    def __init__(self, location):
        super().__init__(location)
        self.eat_count = 0
        self.direction_list = [Direction.NORTH, Direction.EAST, Direction.SOUTH, Direction.WEST,
                               Direction.EAST, Direction.SOUTH, Direction.WEST, Direction.SOUTH, Direction.WEST, Direction.WEST]
        self.move_index = 0

    """Sets the color of the torero as spring green."""
    def get_color(self):
        return "spring green"
    
    """Returns True when it moves either north or south and False if it moves east or west."""
    def eat(self):
        if self.direction_list[self.move_index] ==  Direction.NORTH or self.direction_list[self.move_index] == Direction.SOUTH:
            return True
        else:
            return False
    
    """Sets the attack fight against a lion as scratch but as roar to any other opponent."""
    def fight(self, opponent):
        if opponent == "L":
            return Attack.SCRATCH
        else:
    ...
    """Sets the torrero’s movement to loop north, east, south, west, east, south, west, west."""
    def get_move(self, neighbors):
        move = self.direction_list[self.move_index]
        self.move_index = (self.move_index + 1) % len(self.direction_list)
        return move

    """Returns a string representation of this critter, “T”."""
    def __str__(self):
        return "T"
